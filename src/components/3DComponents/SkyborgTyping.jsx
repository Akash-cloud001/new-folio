...
const SkyborgTyping = forwardRef(( props , ref) => {
  ...
  const [totalAnimTiming, setTotalAnimTiming] = useState(0);
  const [animBreakpoints, setAnimBreakPoints] = useState([0,0,0]);
  // let animBreakpoints = [0, 0, 0];
  function toGetTiming() {
    for (let key in actions) {
      console.log(key, actions[key]);
      if (key === "t-pose" || key === "Animation") continue;
      setTotalAnimTiming(prev=>prev+=actions[key]._clip.duration)
      switch (key) {
        case "sitting":
          setAnimBreakPoints(prev => {
            const newBreakpoints = [...prev];
            newBreakpoints[0] = actions[key]._clip.duration;
        ...
        case "sitToType":
          setAnimBreakPoints(prev => {
            const newBreakpoints = [...prev];
            newBreakpoints[1] = actions[key]._clip.duration;
            return newBreakpoints;
          });
          break;
        case "typing":
          setAnimBreakPoints(prev => {
            const newBreakpoints = [...prev];
            newBreakpoints[2] = actions[key]._clip.duration;
            return newBreakpoints;
          });
          break;
        default:
  ...
  // const {rX, rY, rZ} = useControls("rotations",{
